...
model_save_path = '/storage/research/data/Few_shot/Ai_project/Models/cnn_model.h5'
...
model.add(Conv1D(filters=128, kernel_size=5, activation='relu'))  # 1D Convolutional layer with 128 filters and kernel size 5
...
with open(os.path.join(output_directory, 'classification_report_cnn.txt'), 'w') as f:
...
plt.savefig(os.path.join(output_directory, 'confusion_matrix_cnn.png'))
...
plt.title("ROC Curve")
plt.legend()
plt.savefig(os.path.join(output_directory, 'roc_curve_cnn.png'))
...
plt.ylabel("Accuracy")
plt.legend()
plt.savefig(os.path.join(output_directory, 'accuracy_curve_cnn.png'))
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(os.path.join(output_directory, 'loss_curve_cnn.png'))
...
interface = gr.Interface(
    ...
    outputs="text",
    title="Hope Speech Classifier (1D CNN)",

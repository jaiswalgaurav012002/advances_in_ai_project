...
model_save_path = '/storage/research/data/Few_shot/Ai_project/Models/bilstm_model_small.h5'
...
model = Sequential()
model.add(Bidirectional(LSTM(64, return_sequences=True, input_shape=(X.shape[1], X.shape[2]))))  # 64 units
model.add(Dropout(0.2))
model.add(Bidirectional(LSTM(32)))  # 32 units
...
# Build the model explicitly for `model.summary()` to work
...
with open(os.path.join(output_directory, 'classification_report_BiLSTM.txt'), 'w') as f:
...
plt.savefig(os.path.join(output_directory, 'confusion_matrix_BiLSTM.png'))
...
plt.title("ROC Curve")
plt.legend()
plt.savefig(os.path.join(output_directory, 'roc_curve_BiLSTM.png'))
...
plt.ylabel("Accuracy")
plt.legend()
plt.savefig(os.path.join(output_directory, 'accuracy_curve_BiLSTM.png'))
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(os.path.join(output_directory, 'loss_curve_BiLSTM.png'))

...
model_save_path = '/storage/research/data/Few_shot/Ai_project/Models/lstm_model_small.h5'
...
model = Sequential()
model.add(LSTM(128, return_sequences=True, input_shape=(X.shape[1], X.shape[2])))  # 128 units
model.add(Dropout(0.2))
model.add(LSTM(64))  # 64 units
...
with open(os.path.join(output_directory, 'classification_report_LSTM.txt'), 'w') as f:
...
plt.savefig(os.path.join(output_directory, 'confusion_matrix_LSTM.png'))
...
plt.title("ROC Curve")
plt.legend()
plt.savefig(os.path.join(output_directory, 'roc_curve_LSTM.png'))
...
plt.ylabel("Accuracy")
plt.legend()
plt.savefig(os.path.join(output_directory, 'accuracy_curve_LSTM.png'))
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(os.path.join(output_directory, 'loss_curve_LSTM.png'))
